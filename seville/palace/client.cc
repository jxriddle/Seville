...
namespace seville
{
    namespace palace
    {
        ...
        { //_from_socket_ptr_(
            ...
            auto nBytesToRead = (isPartialRead * nBytesAvailable) |
        ...
        void Client::do_clear(void)
        {
            ...
            //         my_netMsg.clear();
            // my_connectionState = ConnectionState::kDisconnectedState;
        ...
        void Client::on_networkAccessManagerDidFinish(QNetworkReply* replyPtr)
        {
            ...
            if (isErrorReply) {
                my_logger.appendErrorMessage(
                    QString("HTTP network load error: %1")
                    .arg(errorString));
                return;
            }
            ...
            if (contentType == QString("application/json")) {
                ...
                if (url == my_server.httpServerLocation() +
                    ...
                    emit backgroundImageDidLoadEvent();
                         ...
                         my_server.httpPropStorageLocation()) {
                    auto propId = url.toString().rightRef(
                        url.toString().length() -
...
//                  .arg(my_socketPtr->bytesAvailable()));
        ...
        {
            auto isLittleEndian =
                    (Host::ByteOrder::kLittleEndian * !Client::isBigEndian());
            auto isBigEndian =
        ...
        i32 Client::do_determineServerByteOrder(void)
        {
            ...
            auto clientIsBigEndian = (my_byteOrder == Host::ByteOrder::kBigEndian);
            auto serverByteOrderIfServerEndianMismatch =
                    !clientIsBigEndian * Host::ByteOrder::kBigEndian |
        ...
        Host::ByteOrder Client::do_clientByteOrder(void)
        {
            ...
            return static_cast<Host::ByteOrder>(isLittleEndian | isBigEndian);
        }

        Host::ByteOrder Client::do_serverByteOrder(void)
        {
            auto result = Host::ByteOrder::kUnknownEndian;
            // TODO more consideration here.
            // constants are in <s>big</s> little endian byte order,
            // conditionally swapping them if we are a little endian system
            // seems off.
            ...
            switch (kind) {
                ...
                my_logger.appendDebugMessage("Server has Unknown Byte Order");
                // my_server.setByteOrder(Host::ByteOrder::kUnknownEndian);
                result = Host::ByteOrder::kUnknownEndian;
                break;
            case NetMsg::kServerEndianMatchKind:
                my_logger.appendDebugMessage("Server endian matches client");
                // my_server.setByteOrder(my_byteOrder);
            ...
            case NetMsg::kServerEndianMismatchKind:
                ...
                if (clientIsBigEndian) {
                    result = Host::ByteOrder::kLittleEndian;
                }
                else {
                    result = Host::ByteOrder::kBigEndian;
                }
                // result = serverByteOrderIfServerEndianMismatch;
                break;
            default:
                my_logger.appendDebugMessage(
                         "Server did not indicate its Byte Order");
        ...
        */
        ...
        NetMsgOptions Client::do_shouldSwapEndianness(void) const
        {
            auto notUnknownClientByteOrder =
                    my_byteOrder != Host::ByteOrder::kUnknownEndian;

            auto notUnknownServerByteOrder =
                    my_server.byteOrder() != Host::ByteOrder::kUnknownEndian;
        ...
        */
        ...
        {
            auto shouldSwapEndianness = do_shouldSwapEndianness();
            my_registration.setShouldSwapEndiannessFlag(shouldSwapEndianness);
            // my_registration.generate();
            my_registration.setDefault();
            ...
            // netMsg.appendU32(my_registration.regCounter());
            // netMsg.appendU32(my_registration.regCrc());
            ...
            const QString& username, const QString& password)
        {
            ...
            auto authMsg = NetMsg(do_shouldSwapEndianness());
        ...
        {
            auto talkMsg = NetMsg(do_shouldSwapEndianness());
        ...
        {
            auto xTalkMsg = NetMsg(do_shouldSwapEndianness());
        ...
        {
            auto msg = NetMsg(do_shouldSwapEndianness());
        ...
        {
            auto pingMsg = palace::NetMsg(do_shouldSwapEndianness());
        ...
        {
            auto pongMsg = palace::NetMsg(do_shouldSwapEndianness());
        ...
        i32 Client::do_sendGotoRoom(u16 roomId)
        {
            ...
            auto gotoroomMsg = palace::NetMsg(do_shouldSwapEndianness());
