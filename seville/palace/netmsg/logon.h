      ...
      namespace netmsg
      {
         ...
         public:
            ...
            auto reg_crc(void) const -> u32 {
               return u32_at(kRegCrcOffset);
            }

            auto set_reg_crc(u32 value) -> void {
               set_u32_at(kRegCrcOffset, value);
            }

            auto reg_counter(void) const -> u32 {
               return u32_at(kRegCounterOffset);
            }

            auto set_reg_counter(u32 value) -> void {
               set_u32_at(kRegCounterOffset, value);
            }

            auto username(void) const -> QString {
               return pascal_qstring_at(kUsernameOffset);
            }

            auto set_username(const QString& value) -> void {
               set_pascal_qstring_at(kUsernameOffset, value);
            }

            auto username_len(void) const -> u8 {
               return u8_at(kUsernameLenOffset);
            }

            auto set_username_len(u8 value) -> void {
               set_u8_at(kUsernameLenOffset, value);
            }
            ...
            auto set_flags(u32 value) -> void {
               set_u32_at(kFlagsOffset, value);
            }

            auto puid_counter(void) const -> u32 {
               return u32_at(kPuidCounterOffset);
            }

            auto set_puid_counter(u32 value) -> void {
               set_u32_at(kPuidCounterOffset, value);
            }

            auto puid_crc(void) const -> u32 {
               return u32_at(kPuidCrcOffset);
            }

            auto set_puid_crc(u32 value) -> void {
               set_u32_at(kPuidCrcOffset, value);
            }

            auto demo_elapsed(void) const -> u32 {
               return u32_at(kDemoElapsedOffset);
            }

            auto set_demo_elapsed(u32 value) -> void {
               set_u32_at(kDemoElapsedOffset, value);
            }

            auto total_elapsed(void) const -> u32 {
               return u32_at(kTotalElapsedOffset);
            }

            auto set_total_elapsed(u32 value) -> void {
               set_u32_at(kTotalElapsedOffset, value);
            }

            auto demo_limit(void) const -> u32 {
               return u32_at(kDemoLimitOffset);
            }

            auto set_demo_limit(u32 value) -> void {
               set_u32_at(kDemoLimitOffset, value);
            }

            auto initial_room_id(void) const -> u16 {
               return u16_at(kInitialRoomIdOffset);
            }

            auto set_initial_room_id(u16 value) -> void {
               set_u16_at(kInitialRoomIdOffset, value);
            }

            auto reserved(void) const -> QString {
               return qstring_at(kReservedOffset, kIdentSize);
            }

            auto set_reserved(const QString& value) -> void {
               set_qstring_at(kReservedOffset, value);
            }

            auto upload_requested_protocol_version(void) const -> u32 {
               return u32_at(kUploadRequestedProtocolVersionOffset);
            }
            ...
            auto upload_capabilities(void) const -> u32 {
               return u32_at(kUploadCapabilitiesOffset);
            }

            auto set_upload_capabilities(u32 value) -> void {
               set_u32_at(kUploadCapabilitiesOffset, value);
            }

            auto download_capabilities(void) const -> u32 {
               return u32_at(kDownloadCapabilitiesOffset);
            }

            auto set_download_capabilities(u32 value) -> void {
               set_u32_at(kDownloadCapabilitiesOffset, value);
            }

            auto engine_capabilities_2d(void) const -> u32 {
               return u32_at(kEngineCapabilities2dOffset);
            }

            auto set_engine_capabilities_2d(u32 value) -> void {
               set_u32_at(kEngineCapabilities2dOffset, value);
            }

            auto graphics_capabilities_2d(void) const -> u32 {
               return u32_at(kGraphicsCapabilities2dOffset);
            }

            auto set_graphics_capabilities_2d(u32 value) -> void {
               set_u32_at(kGraphicsCapabilities2dOffset, value);
            }

            auto graphics_capabilities_3d(void) const -> u32 {
               return u32_at(kGraphicsCapabilities3dOffset);
            }

            auto set_graphics_capabilities_3d(u32 value) -> void {
               set_u32_at(kGraphicsCapabilities3dOffset, value);
            }
